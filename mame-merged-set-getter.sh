...
if [[ "${INSTALL^^}" == "TRUE" ]] && [ ! -e "/media/fat/Scripts/update_mame-getter.sh" ]
   then
      ...
      echo ""
      curl ${CURL_RETRY} ${SSL_SECURITY_OPTION} --location -o "/media/fat/Scripts/update_mame-getter.sh" https://raw.githubusercontent.com/kaloun34/MiSTer_SCRIPTS/master/mame-merged-set-getter.sh || true
      echo
fi
